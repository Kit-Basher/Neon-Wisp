...
const MusicSystem: React.FC<MusicSystemProps> = ({ score, isLocked, isGameOver }) => {
  ...
  useEffect(() => {
    ...
    bitCrusher.current = new Tone.BitCrusher(6).connect(volume.current);
    ...
    // Volume set to 3 for balance
    bassSynth.current.volume.value = 3;
    ...
    leadSynth.current.volume.value = -4;
